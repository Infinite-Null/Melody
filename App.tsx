...
function App(){
  ...
  function StatusOfCodePush(status:string){
    console.log(status);
  }
  var onDownloadProgress = function (downloadProgress:any) {
    if (downloadProgress) {
  ...
  useEffect(()=>{
    // @ts-ignore
    CodePush.sync({installMode:CodePush.InstallMode.IMMEDIATE, updateDialog:true},StatusOfCodePush,onDownloadProgress)
  })
